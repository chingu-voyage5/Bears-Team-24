...
function HTMLToJSON(HTMLString) {
  ...
  const resultObj = {};
  const structure = detectElements(HTMLString);
...
function detectElements(html) {
  ...
  const tags = html.match(TAG_DETECT);
  ...
  if (!tags) {
    ...
    const smartHTML = html.replace(TAG_DETECT, '<>');
    let pos = 0;
    let count = 0;
    while (pos < smartHTML.length) {
      const endString = smartHTML.slice(pos);
      if (endString.startsWith('<>')) {
        elements.push(tags[count]);
        count++;
        pos += 2;
      } else {
        const stop = endString.indexOf('<>');

        if (stop == -1) {
          elements.push(endString);
          break;
        } else {
          const text = endString.slice(0, stop);
          if (text.length > 0) {
            elements.push(text);
          }
          pos += stop;
  ...
  for (let i = 0; i < elements.length; i++) {
    ...
    if (tree[i].children) {
      const innerHTML = tree[i]['inner HTML'];
...
function unwrap(html) {
  ...
  const stop = temp.indexOf('>');
  const openingTag = temp
    .slice(1, stop)
    .replace('/S+/g', ' ')
    .split(' ');
  ...
  for (let j = 1; j < openingTag.length; j++) {
    const [attr, val] = openingTag[j].split('=');
